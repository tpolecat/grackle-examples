...
object TimeMapping2 extends ExampleMain:
  def mapping = Resource.pure(new TimeMapping2[IO])

// add a parameterized field
class TimeMapping2[F[_]: Monad] extends ValueMapping[F]:
  ...
  override val selectElaborator: SelectElaborator =
    SelectElaborator(Map(
      QueryType -> {
        case Select("timeZone", List(Binding("id", StringValue(id))), child) =>
          Result(Select("timeZone", Nil, Unique(Filter(Eql(TimeZoneType / "id", Const(id)), child))))
        case Select("timeZone", List(Binding("id", other)), child) =>
          Result.failure(s"id: expected a string, got $other")
      }
    ))
